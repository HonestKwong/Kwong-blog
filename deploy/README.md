...
- `docker-compose.kwong.yml`：只定义 `kwong-web`，加入外部网络 `mynetwork`
...
`KWONG_DOCKER_NETWORK` 默认 `mynetwork`，必须与 `xray-deploy` 的 Compose 网络同名，nginx 才能通过 `http://kwong-web:3000` 访问网站。
